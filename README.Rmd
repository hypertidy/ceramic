...
pt <- cbind(136, -34)
im <- cc_location(pt, buffer = c(1e6, 5e5), type = "satellite-v9")
library(raster)
plotRGB(im)

## get the matching tiles (zoom is magic here, it's all wrapped - needs thought)
get_tiles
tiles <- ceramic_tiles(zoom = 6, type = "satellite-v9")
library(sf)
...
This code downloads a specific region as elevation and imagery at specific zoom levels to build [a 3D scene](https://hypertidy.github.io/anglr-demos/ceramic-demo01.html). 
...
```{r files}
aa <- cc_location(loc = cbind(0, 0), buffer = 330000, type = "satellite-v9")
ceramic_tiles(zoom = 7, type = "satellite-v9")
```
