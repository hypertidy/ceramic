...
#> Warning in seq.default(along = band): partial argument match of 'along' to
#> 'along.with'
...
plotRGB(im)
#> Warning in grDevices::rgb(RGB[, 1], RGB[, 2], RGB[, 3], alpha = alpha, max
#> = scale): partial argument match of 'max' to 'maxColorValue'
...
#> Warning in seq.default(xmin(x), xmax(x), length = ncol(x) + 1L): partial
#> argument match of 'length' to 'length.out'
#> Warning in seq.default(ymax(x), ymin(x), length = nrow(x) + 1L): partial
#> argument match of 'length' to 'length.out'
#> Warning in seq.default(0, length = nrow(x), by = ncol(x) + 1): partial
#> argument match of 'length' to 'length.out'
middle <- function(x, y) {
  x + (y - x)/2
}
text(middle(tiles$xmin, tiles$xmax), middle(tiles$ymin, tiles$ymax), lab = sprintf("[%i,%i]", tiles$tile_x, tiles$tile_y), 
     col = "firebrick")
#> Warning in text.default(middle(tiles$xmin, tiles$xmax),
#> middle(tiles$ymin, : partial argument match of 'lab' to 'labels'
```
